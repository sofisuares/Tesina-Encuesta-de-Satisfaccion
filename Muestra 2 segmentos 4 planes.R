...
### STRATUM
STRATUM3 <- read_excel("STRATUM - 2 segmentos 4 planes.xlsx", 
                       sheet = "STRATUM")
STRATUM3 <- as.data.frame(STRATUM3) #necesario que sea dataframe
...
allocation_1 <- beat.1st(stratif=STRATUM3, errors= cv1, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_1$sensitivity
sum(allocation_1$n) #6449
...
allocation_2 <- beat.1st(stratif=STRATUM3, errors= cv2, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_2$sensitivity
sum(allocation_2$n) #5582
...
allocation_3 <- beat.1st(stratif=STRATUM3, errors= cv3, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_3$sensitivity
sum(allocation_3$n) #3537
...
allocation_4 <- beat.1st(stratif=STRATUM3, errors= cv4, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_4$sensitivity
sum(allocation_4$n) #3128
...
allocation_5 <- beat.1st(stratif=STRATUM3, errors= cv5, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_5$sensitivity
sum(allocation_5$n) #3052
...
allocation_6 <- beat.1st(stratif=STRATUM3, errors= cv6, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_6$sensitivity
sum(allocation_6$n)  #2714
...
allocation_7 <- beat.1st(stratif=STRATUM3, errors= cv7, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_7$sensitivity
sum(allocation_7$n) #5582
...
allocation_8 <- beat.1st(stratif=STRATUM3, errors= cv8, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_8$sensitivity
sum(allocation_8$n) #3526
...
allocation_9 <- beat.1st(stratif=STRATUM3, errors= cv9, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_9$sensitivity
sum(allocation_9$n) #3116
...
allocation_10 <- beat.1st(stratif=STRATUM3, errors= cv10, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_10$sensitivity
sum(allocation_10$n) #3038
...
allocation_11 <- beat.1st(stratif=STRATUM3, errors= cv11, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_11$sensitivity
sum(allocation_11$n) #1844
...
allocation_12 <- beat.1st(stratif=STRATUM3, errors= cv12, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_12$sensitivity
sum(allocation_12$n) #1820
...
allocation_13 <- beat.1st(stratif=STRATUM3, errors= cv13, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_13$sensitivity
sum(allocation_13$n) #1691
...
allocation_14 <- beat.1st(stratif=STRATUM3, errors= cv14, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_14$sensitivity
sum(allocation_14$n) #1347
...
STRATUM3b <- allocation_14$file_strata #agregar columna n a STRATUM con adjudicación
...
STRATUM3b$ninflado <- ifelse(STRATUM3b$N <= STRATUM3b$n, STRATUM3b$n, STRATUM3b$n / 0.5)
sum(STRATUM3b$ninflado) #TOTAL MUESTRA: 2644 GF
...
Marco3 <- read_excel("STRATUM - 2 segmentos 4 planes.xlsx", 
                     sheet = "MM transformado")
Marco3 <- as.data.frame(Marco3) #N=88.839 GF

### Ordeno STRATUM y Marco según estratos para que coincida el nombre del estrato 
# en Marco3 con el tamaño muestral en Stratum1b

#Ordeno STRATUM3b ascendente según estrato
STRATUM3b <- STRATUM3b[order(STRATUM3b$ESTRATOS),]

#Ordeno Marco3 ascendente según estrato
Marco3 <- Marco3[order(Marco3$ESTRATOS),]

Marco3$ESTRATOS <- factor(Marco3$ESTRATOS)

### Obtener muestra: listado GF

Muestra3 <- strata(Marco3, stratanames= c("ESTRATOS"),size=STRATUM3b$ninflado, method="srswor")
summary(Muestra3$ESTRATOS) #Controlar que haya el tamaño de muestra requerido en cada estrato
Muestra <- Marco3[Muestra3$ID_unit,]  #Para obtener el número de GF. De Marco1 me quedo con las filas que indica ID_unit


### Exportar muestra a excel
#install.packages("openxlsx")
library(openxlsx)
write.xlsx(Muestra, "Muestra2SegmentosConCerrado.xlsx")
