...
### STRATUM
STRATUM2 <- read_excel("STRATUM - 5 segmentos 3 planes.xlsx", 
                       sheet = "STRATUM")
STRATUM2 <- as.data.frame(STRATUM2) #necesario que sea dataframe
...
allocation_1 <- beat.1st(stratif=STRATUM2, errors= cv1, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_1$sensitivity
sum(allocation_1$n) #6682
...
allocation_2 <- beat.1st(stratif=STRATUM2, errors= cv2, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_2$sensitivity
sum(allocation_2$n) #5500
...
allocation_3 <- beat.1st(stratif=STRATUM2, errors= cv3, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_3$sensitivity
sum(allocation_3$n) #3760
...
allocation_4 <- beat.1st(stratif=STRATUM2, errors= cv4, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_4$sensitivity
sum(allocation_4$n) #3175
...
allocation_5 <- beat.1st(stratif=STRATUM2, errors= cv5, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_5$sensitivity
sum(allocation_5$n) #3091
...
allocation_6 <- beat.1st(stratif=STRATUM2, errors= cv6, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_6$sensitivity
sum(allocation_6$n)  #2860
...
allocation_7 <- beat.1st(stratif=STRATUM2, errors= cv7, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_7$sensitivity
sum(allocation_7$n) #5500
...
allocation_8 <- beat.1st(stratif=STRATUM2, errors= cv8, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_8$sensitivity
sum(allocation_8$n) #3760
...
allocation_9 <- beat.1st(stratif=STRATUM2, errors= cv9, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_9$sensitivity
sum(allocation_9$n) #3175
...
allocation_10 <- beat.1st(stratif=STRATUM2, errors= cv10, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_10$sensitivity
sum(allocation_10$n) #3091
...
allocation_11 <- beat.1st(stratif=STRATUM2, errors= cv11, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_11$sensitivity
sum(allocation_11$n) #2078
...
allocation_12 <- beat.1st(stratif=STRATUM2, errors= cv12, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_12$sensitivity
sum(allocation_12$n) #2054
...
allocation_13 <- beat.1st(stratif=STRATUM2, errors= cv13, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_13$sensitivity
sum(allocation_13$n) #1875
...
allocation_14 <- beat.1st(stratif=STRATUM2, errors= cv14, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_14$sensitivity
sum(allocation_14$n) #1582
...
STRATUM2b <- allocation_14$file_strata #agregar columna n a STRATUM con adjudicación
...
STRATUM2b$ninflado <- ifelse(STRATUM2b$N <= STRATUM2b$n, STRATUM2b$n, STRATUM2b$n / 0.5)
sum(STRATUM2b$ninflado) #TOTAL MUESTRA: 2968 GF
...
Marco2 <- read_excel("STRATUM - 5 segmentos 3 planes.xlsx", 
                     sheet = "MM transformado")
Marco2 <- as.data.frame(Marco2) #N=87.148 GF

### Ordeno STRATUM y Marco según estratos para que coincida el nombre del estrato 
# en Marco2 con el tamaño muestral en Stratum1b

#Ordeno STRATUM2b ascendente según estrato
STRATUM2b <- STRATUM2b[order(STRATUM2b$ESTRATOS),]

#Ordeno Marco2 ascendente según estrato
Marco2 <- Marco2[order(Marco2$ESTRATOS),]

Marco2$ESTRATOS <- factor(Marco2$ESTRATOS)

### Obtener muestra: listado GF

Muestra2 <- strata(Marco2, stratanames= c("ESTRATOS"),size=STRATUM2b$ninflado, method="srswor")
summary(Muestra2$ESTRATOS) #Controlar que haya el tamaño de muestra requerido en cada estrato
Muestra <- Marco2[Muestra2$ID_unit,]  #Para obtener el número de GF. De Marco1 me quedo con las filas que indica ID_unit


### Exportar muestra a excel
#install.packages("openxlsx")
library(openxlsx)
write.xlsx(Muestra, "Muestra5SegmentosSinCerrado.xlsx")
