...
### STRATUM
STRATUM1 <- read_excel("STRATUM - 5 segmentos 4 planes.xlsx", 
                       sheet = "STRATUM")
STRATUM1 <- as.data.frame(STRATUM1) #necesario que sea dataframe
...
allocation_1 <- beat.1st(stratif=STRATUM1, errors= cv1, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_1$sensitivity
sum(allocation_1$n) #7218
...
allocation_2 <- beat.1st(stratif=STRATUM1, errors= cv2, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_2$sensitivity
sum(allocation_2$n) #5647
...
allocation_3 <- beat.1st(stratif=STRATUM1, errors= cv3, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_3$sensitivity
sum(allocation_3$n) #4122
...
allocation_4 <- beat.1st(stratif=STRATUM1, errors= cv4, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_4$sensitivity
sum(allocation_4$n) #3382
...
allocation_5 <- beat.1st(stratif=STRATUM1, errors= cv5, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_5$sensitivity
sum(allocation_5$n) #3237
...
allocation_6 <- beat.1st(stratif=STRATUM1, errors= cv6, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_6$sensitivity
sum(allocation_6$n)  #2990
...
allocation_7 <- beat.1st(stratif=STRATUM1, errors= cv7, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_7$sensitivity
sum(allocation_7$n) #5647
...
allocation_8 <- beat.1st(stratif=STRATUM1, errors= cv8, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_8$sensitivity
sum(allocation_8$n) #4123
...
allocation_9 <- beat.1st(stratif=STRATUM1, errors= cv9, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_9$sensitivity
sum(allocation_9$n) #3382 
...
allocation_10 <- beat.1st(stratif=STRATUM1, errors= cv10, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_10$sensitivity
sum(allocation_10$n) #3237
...
allocation_11 <- beat.1st(stratif=STRATUM1, errors= cv11, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_11$sensitivity
sum(allocation_11$n) #2312
...
allocation_12 <- beat.1st(stratif=STRATUM1, errors= cv12, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_12$sensitivity
sum(allocation_12$n) #2292
...
allocation_13 <- beat.1st(stratif=STRATUM1, errors= cv13, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_13$sensitivity
sum(allocation_13$n) #2062
...
allocation_14 <- beat.1st(stratif=STRATUM1, errors= cv14, minnumstrat=2, maxiter=200, maxiter1=25, epsilon=10^(-11))
allocation_14$sensitivity
sum(allocation_14$n) #1772
...
STRATUM1b <- allocation_14$file_strata #agregar columna n a STRATUM con adjudicación

############################################
###   Se calcula una muestra inflada     ###
### suponiendo tasa de respuesta del 50% ###
############################################

### Creación de la nueva columna "ninflado" en STRATUM. Condición para que en los 
# casos en los que N sea menor o igual a 10, n inflado sea igual a n (por ser un censo)

STRATUM1b$ninflado <- ifelse(STRATUM1b$N <= STRATUM1b$n, STRATUM1b$n, STRATUM1b$n / 0.5)
sum(STRATUM1b$ninflado) #TOTAL MUESTRA: 3284 GF
...
Marco1 <- read_excel("STRATUM - 5 segmentos 4 planes.xlsx", 
                     sheet = "MM transformado")
Marco1 <- as.data.frame(Marco1) #N=88.839 GF

### Ordeno STRATUM y Marco según estratos para que coincida el nombre del estrato 
# en Marco1 con el tamaño muestral en Stratum1b

#Ordeno STRATUM1b ascendente según estrato
STRATUM1b <- STRATUM1b[order(STRATUM1b$ESTRATOS),]

#Ordeno Marco1 ascendente según estrato
Marco1 <- Marco1[order(Marco1$ESTRATOS),]

Marco1$ESTRATOS <- factor(Marco1$ESTRATOS)

### Obtener muestra: listado GF

Muestra1 <- strata(Marco1, stratanames= c("ESTRATOS"),size=STRATUM1b$ninflado, method="srswor")
summary(Muestra1$ESTRATOS) #Controlar que haya el tamaño de muestra requerido en cada estrato
Muestra <- Marco1[Muestra1$ID_unit,]  #Para obtener el número de GF. De Marco1 me quedo con las filas que indica ID_unit


### Exportar muestra a excel
#install.packages("openxlsx")
library(openxlsx)
write.xlsx(Muestra, "Muestra5SegmentosConCerrado.xlsx")